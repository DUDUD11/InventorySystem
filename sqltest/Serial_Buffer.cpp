...
{
	int idx = Buffer_Find();
	if (m_Buffer_struct[idx].inputDataSize + dynamic_.len > m_BufferSize) __debugbreak();
...
{
	int idx = __f();
	if (__s()readDataSize + dynamic_.len > __s()inputDataSize) __debugbreak();
...
CPacket& CPacket::operator<<(D_Packet dynamic_)
{
	...
	if (m_inputDataSize + dynamic_.len > m_BufferSize) __debugbreak();
...
CPacket& CPacket::operator>>(D_Packet& dynamic_)
{
	if (m_readDataSize + dynamic_.len > m_inputDataSize) __debugbreak();
